...
public interface IDevoxDataSource
{
    ...
    Task<IEnumerable<DevoxPost>> GetPosts();
    Task<IEnumerable<DevoxPost>> GetMoreVoxes(int count);
}

public class DevoxDataSource : IDevoxDataSource, IPostSource<DevoxPost>
{
    ...
    public async Task<IEnumerable<DevoxPost>> GetPosts()
    {

        //var options = new RestClientOptions(Constants.ApiUrl)
        //{
        //    Proxy = proxy,
        //    MaxTimeout = -1,
        ...
        var request = new RestRequest(Constants.ApiUrl + Constants.Enpoint, Method.Post);
